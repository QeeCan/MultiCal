...
public class MonthView extends ViewGroup {
    ...
    public void setDateList(List<DateBean> dates, int currentMonthDays) {
        ...
        for (int i = 0; i < dates.size(); i++) {
            ...
            ImageView cloclInStatus = null;//原点图片，表示是否打卡-正常或异常
            if (item_layout != 0 && calendarViewAdapter != null) {
                ...
                cloclInStatus = (ImageView) view.findViewById(R.id.clockInStatusIV);
            }
            /**
             * 处理状态
             */
            if (cloclInStatus != null && date.getClickInStatus() == 2) {
                cloclInStatus.setImageDrawable(mContext.getResources().getDrawable(R.drawable.red_point));
                cloclInStatus.setVisibility(VISIBLE);
            } else if (cloclInStatus != null && date.getClickInStatus() == 1) {
                cloclInStatus.setImageDrawable(mContext.getResources().getDrawable(R.drawable.gray_point));
                cloclInStatus.setVisibility(VISIBLE);
            } else {
                //不需要展示
            }
